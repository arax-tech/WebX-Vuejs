...
const refForm = ref()
const ValidateFunction1 = (event) => {
    event.preventDefault();
    
    refForm?.value?.validate().then(({ valid: isValid }) => {
        if (isValid) UpdateSettingFunction1();
        else console.log('error')
    })
}


const UpdateSettingFunction1 = async (event) => {
    event.preventDefault();
        ...
        <VCol cols="12">
            <VCard title="Setting">
                ...
                <VCardText class="pt-0">
                    <!-- 👉 Form -->
                    <VForm class="mt-6" @submit.prevent="UpdateSettingFunction1">
                        <VRow>
                            ...
                            <VCol md="3" cols="12">
                                <AppTextField prepend-inner-icon="tabler-phone" v-model="data.phone1"
                                    placeholder="Phone1" persistent-placeholder label="Phone1"
                                    :rules="[requiredValidator]" />
                            </VCol>
                            <VCol md="3" cols="12">
                                <AppTextField prepend-inner-icon="tabler-phone" v-model="data.phone2"
                                    placeholder="Phone2" persistent-placeholder label="Phone2"
                                    :rules="[requiredValidator]" />
                            </VCol>
                            <VCol md="4" cols="12">
                                <AppTextField prepend-inner-icon="tabler-phone" v-model="data.phone3"
                                    placeholder="Phone3" persistent-placeholder label="Phone3"
                                    :rules="[requiredValidator]" />
                            </VCol>
                            <VCol md="4" cols="12">
